...
def memory_file_list():
	...
	for root, dirs, files in os.walk(memory_rule_dir):
		for file1 in tqdm(files,desc="遍历所有内存扫描规则"):
			time.sleep(0.1)
...
def win_pid():
    ...
    if int(pid_value) == 0:
        for process in tqdm(processes,desc="提取java,python,php进程"):
    ...
    elif int(pid_value) == 1:
        for allpid in tqdm(processes,desc="提取所有进程"):
            try:
                win_pid_list.append(allpid)
            except psutil.NoSuchProcess:
                print(f"PID: {process} not found")

    else:
        print("配置文件pid_value字段只允许0/1")
...
def win_memory():
	...
	#定义扫描结果列表
	memory_list = []
	for mi in tqdm(memory_rule_file,desc="遍历所有内存规则进行扫描"):
		time.sleep(0.1)
		for mj in tqdm(memory_pid,desc="遍历win_pid提取的进程进行扫描"):
			time.sleep(0.1)
			#判断是否打印字符串
			if int(mode) == 0:
